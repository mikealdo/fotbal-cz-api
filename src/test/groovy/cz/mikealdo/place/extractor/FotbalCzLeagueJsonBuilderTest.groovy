...
class FotbalCzLeagueJsonBuilderTest extends Specification {
    ...
    def "should produce valid json result"() {
        given:
            ...
            MatchResult result = new MatchResult("2:1", "(PK:5:4)")
            matches.add(new FotbalCzMatch(DateTime.parse("2015-05-15"), firstTeam, secondTeam, result, 1))
            league.setMatches(matches)
        when:
            String json = builder.buildLeagueJson("hash", league)
        then:
            assertThat(json, equalsReferenceJson(Results.FULL_RESULT))
